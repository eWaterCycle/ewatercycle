...
class Lisflood(AbstractModel):
    ...
    def _create_lisflood_config(self, work_dir: Path, start_time_iso: str = None, end_time_iso: str = None, IrrigationEfficiency: str = None) -> Path:
        ...
        settings = {
            ...
            "MaskMap": f"{self.parameter_set.MaskMap.stem}",
        ...
        for textvar in self.cfg.config.iter("textvar"):
            ...
            maps_prefixes = {
                'E0Maps': {'name': 'PrefixE0', 'value': f"{Path(self.forcing.PrefixE0).stem}"},
                'ES0Maps': {'name': 'PrefixES0', 'value': f"{Path(self.forcing.PrefixES0).stem}"},
                'ET0Maps': {'name': 'PrefixET0', 'value': f"{Path(self.forcing.PrefixET0).stem}"},
            }
            # output of lisvap
